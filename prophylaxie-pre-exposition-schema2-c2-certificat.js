// prophylaxie-pre-exposition-schema2-c2-certificat.js
...
window.addEventListener('certDataReady', function () {
    ...
    const saveButton = document.getElementById('saveSchema2C2Button');
    if (saveButton) {
        saveButton.addEventListener('click', function () {
            console.log("Clic sur le bouton de sauvegarde Avec ATCD Vaccinaux (Schéma 2).");
            ...
            if (typeof envoyerMessageNatifRabique === 'function') {
                ...
                envoyerMessageNatifRabique(data)
                    .then(() => {
                        console.log("Sauvegarde du certificat Avec ATCD Vaccinaux (Schéma 2) réussie.");
                        alert("✅ Succès !\n\nLe certificat antirabique (Avec ATCD Vaccinaux (Schéma 2)) a été sauvegardé.");
                    })
                    .catch(err => {
                        console.error("Erreur lors de la sauvegarde du certificat Avec ATCD Vaccinaux (Schéma 2):", err);
