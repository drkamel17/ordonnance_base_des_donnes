...
// Fonction pour mettre à jour la taille de police dans le certificat d'arrêt de travail
function updateFontSizeInArretTravail(fontSize) {
    ...
    const oldStyle = document.getElementById('arretTravailFontSizeStyle');
    if (oldStyle) {
        oldStyle.remove();
    }

    // Ajouter le nouveau style
    styleElement.id = 'arretTravailFontSizeStyle';
    document.head.appendChild(styleElement);

    // Stocker la taille de police dans localStorage
    localStorage.setItem('arretTravailFontSize', fontSize);
}

// Attendre que le document soit chargé
document.addEventListener('DOMContentLoaded', () => {
    // Charger la taille de police sauvegardée
    const savedFontSize = localStorage.getItem('arretTravailFontSize') || '14';
    ...
    if (fontSizeInput) {
        ...
        fontSizeInput.addEventListener('input', () => {
            ...
            updateFontSizeInArretTravail(fontSize);
        });

        // Appliquer la taille de police initiale
        updateFontSizeInArretTravail(savedFontSize);
    }
});
