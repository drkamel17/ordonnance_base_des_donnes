...
window.addEventListener('certDataReady', function () {
    ...
    document.getElementById('saveZagreb3Button').addEventListener('click', function () {
        ...
        if (typeof envoyerMessageNatifRabique === 'function') {
            envoyerMessageNatifRabique(data)
                .then(() => {
                    alert("✅ Succès !\n\nLe certificat antirabique (Zagreb3) a été sauvegardé.");
                })
                .catch(err => {
                    console.error("Erreur lors de la sauvegarde du certificat Zagreb3:", err);
