// schema-tissulaire-sans-sar-certificat.js
...
window.addEventListener('certDataReady', function () {
    ...
    document.getElementById('saveSchemaTissulaireSansSARButton').addEventListener('click', function () {
        console.log("Clic sur le bouton de sauvegarde Schéma Tissulaire sans SAR.");
        ...
        if (typeof envoyerMessageNatifRabique === 'function') {
            ...
            envoyerMessageNatifRabique(data)
                .then(() => {
                    console.log("Sauvegarde du certificat Schéma Tissulaire sans SAR réussie.");
                    alert("✅ Succès !\n\nLe certificat antirabique (Schéma Tissulaire sans SAR) a été sauvegardé.");
                })
                .catch(err => {
                    console.error("Erreur lors de la sauvegarde du certificat Schéma Tissulaire sans SAR:", err);
