...
// Fonction pour mettre à jour la taille de police dans le certificat Radiox
function updateFontSizeInRadiox(fontSize) {
    ...
    const oldStyle = document.getElementById('radioxFontSizeStyle');
    if (oldStyle) {
        oldStyle.remove();
    }

    // Ajouter le nouveau style
    styleElement.id = 'radioxFontSizeStyle';
    document.head.appendChild(styleElement);

    // Stocker la taille de police dans localStorage
    localStorage.setItem('radioxFontSize', fontSize);
}

// Attendre que le document soit chargé
document.addEventListener('DOMContentLoaded', () => {
    // Charger la taille de police sauvegardée
    const savedFontSize = localStorage.getItem('radioxFontSize') || '14';
    ...
    if (fontSizeInput) {
        ...
        fontSizeInput.addEventListener('input', () => {
            ...
            updateFontSizeInRadiox(fontSize);
        });

        // Appliquer la taille de police initiale
        updateFontSizeInRadiox(savedFontSize);
    }
});
