...
// Fonction pour mettre à jour la taille de police dans le certificat de justification
function updateFontSizeInJustification(fontSize) {
    ...
    const oldStyle = document.getElementById('justificationFontSizeStyle');
    if (oldStyle) {
        oldStyle.remove();
    }

    // Ajouter le nouveau style
    styleElement.id = 'justificationFontSizeStyle';
    document.head.appendChild(styleElement);

    // Stocker la taille de police dans localStorage
    localStorage.setItem('justificationFontSize', fontSize);
}

// Attendre que le document soit chargé
document.addEventListener('DOMContentLoaded', () => {
    // Charger la taille de police sauvegardée
    const savedFontSize = localStorage.getItem('justificationFontSize') || '14';
    ...
    if (fontSizeInput) {
        ...
        fontSizeInput.addEventListener('input', () => {
            ...
            updateFontSizeInJustification(fontSize);
        });

        // Appliquer la taille de police initiale
        updateFontSizeInJustification(savedFontSize);
    }
});
