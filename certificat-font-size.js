...
function updateFontSizeInCertificat(fontSize) {
    ...
    const oldStyle = document.getElementById('certificatFontSizeStyle');
    if (oldStyle) {
        oldStyle.remove();
    }
    ...
    localStorage.setItem('certificatFontSize', fontSize);
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (fontSizeInput) {
        ...
        fontSizeInput.addEventListener('input', () => {
            ...
            updateFontSizeInCertificat(fontSize);
        });

        // Appliquer la taille de police initiale
        updateFontSizeInCertificat(savedFontSize);
    }
});
